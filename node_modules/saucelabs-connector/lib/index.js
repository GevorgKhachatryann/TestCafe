...
var _sauceStorage = _interopRequireDefault(require("./sauce-storage"));

var _sauceHost = require("./sauce-host");

var _isIe = _interopRequireDefault(require("./utils/is-ie11"));

var _messages = require("./messages");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const PRERUN_SCRIPT_DIR_PATH = (0, _readFileRelative.toAbsPath)('./prerun/');
...
const SAUCE_CONNECT_OPTIONS_DENYLIST = ['createTunnel', 'connectorLogging']; // NOTE: When using Appium on Android devices, the device browser navigates to 'https://google.com' after being started.
...
const createSauceConnect = (0, _util.promisify)(_sauceConnectLauncher.default);
const disposeSauceConnect = (0, _util.promisify)((process, ...args) => process.close(...args));
...
class SaucelabsConnector {
  constructor(username, accessKey, options = {}) {
    ...
    this.sauceStorage = new _sauceStorage.default(this.username, this.accessKey);
  ...
  static _getFilteredSauceConnectOptions(options) {
    return Object.keys(options).filter(key => SAUCE_CONNECT_OPTIONS_DENYLIST.indexOf(key) === -1).reduce((obj, key) => {
      obj[key] = options[key];
      return obj;
    }, {});
  }
  ...
  async getSessionUrl(browser) {
    ...
    return `https://app.${_sauceHost.SAUCE_API_HOST}/tests/${sessionId}`;
  }
  ...
  async startBrowser(browser, url, jobOptions = {}, timeout = null) {
    ...
    const webDriver = _wd.default.promiseChainRemote(`ondemand.${_sauceHost.SAUCE_API_HOST}`, SAUCE_API_PORT, this.username, this.accessKey);
    ...
    webDriver.once('status', () => {
      ...
      if (this.options.connectorLogging) {
        this.getSessionUrl(webDriver).then(sessionUrl => this._log((0, _messages.getText)(_messages.MESSAGE.browserStarted, {
    ...
    const initParams = { ...additionalOptions,
      idleTimeout,
      ...(name && {
        name
      }),
    ...
    if ((0, _isIe.default)(browser)) {
      // NOTE: We should upload the script to the sauce storage if it's not there yet.
      if (!(await this.sauceStorage.isFileAvailable(DISABLE_COMPATIBILITY_MODE_IE_SCRIPT_FILENAME))) await this.sauceStorage.uploadFile(PRERUN_SCRIPT_DIR_PATH, DISABLE_COMPATIBILITY_MODE_IE_SCRIPT_FILENAME);
      initParams.prerun = `sauce-storage:${DISABLE_COMPATIBILITY_MODE_IE_SCRIPT_FILENAME}`;
    }

    await webDriver.init(initParams).get(url);
    return webDriver;
  }

  async stopBrowser(browser) {
    clearInterval(browser.pingIntervalId);
    await browser.quit().sauceJobStatus();
  }
  ...
  async disconnect() {
    if (this.sauceConnectProcess) await disposeSauceConnect(this.sauceConnectProcess);
  }
  ...
  async waitForFreeMachines(machineCount, requestInterval, maxAttemptCount) {
    ...
    while (attempts++ < maxAttemptCount) {
      ...
      if (freeMachineCount >= machineCount) return;

      this._log((0, _messages.getText)(_messages.MESSAGE.freeMachinesNumberIsLessThanRequested, {
        freeMachineCount,
        machineCount
      }));

      await (0, _wait.default)(requestInterval);
    }

    throw new Error(_messages.MESSAGE.noFreeMachines);
  }
